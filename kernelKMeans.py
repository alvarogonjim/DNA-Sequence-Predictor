...
class Kernel_KMeans(object):
    ...
    def kernel_distance(self, x, classification):
        ...
        res = (
            self.kernel(x, x)
            - 2 * np.sum([self.kernel(x, j) for j in self.classes[classification]]) / l
            + self.factor[classification]
        )
    ...
    def fit(self, X, y):
        ...
        for ii in range(self.k):
            ...
            self.factor[ii] = np.sum(
                [
                    np.sum([self.kernel(i, j) for j in self.classes[ii]])
                    for i in self.classes[ii]
                ]
            ) / (l ** 2)
    ...
    def predict(self, X):
        ...
        for i, features in enumerate(X):
            distances = [
                self.kernel_distance(features, classification)
                for classification in range(0, len(self.classes))
            ]
