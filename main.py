...
print(style.mono + "The project aim to predict whether a DNA sequencies " \
    ...
    + "are needed to run the file: " + style.normal)
...
if  not validation_set.empty:
    ...
    predictions_valid = model0.predict(gm, np.shape(train_set)[0], \
...
predictions_0 = model0.predict(gm, np.shape(train_set)[0], \
    np.shape(test_set)[0])
...
if  not validation_set.empty:
    # Compute the performances on the validation set
    print("\t\tCompute the performances on the validation set")
    predictions_valid = model1.predict(gm, np.shape(train_set)[0], \
        np.shape(validation_set)[0])
...
print(style.italic + style.blue + "\tPredict on test set" + style.normal)
predictions_1 = model1.predict(gm, np.shape(train_set)[0], \
    np.shape(test_set)[0])
...
if  not validation_set.empty:
    # Compute the performances on the validation set
    print("\t\tCompute the performances on the validation set")
    predictions_valid = model2.predict(gm, np.shape(train_set)[0], \
        np.shape(validation_set)[0])
...
# Predict on test set
print(style.italic + style.blue + "\tPredict on test set" + style.normal)
predictions_2 = model2.predict(gm, np.shape(train_set)[0], \
    np.shape(test_set)[0])
...
final_predicitons.to_csv("Yte_GONZALEZ_LAURENDEAU_kaggle_submission2.csv", \
