...
for step in range(N):
    ...
    print("\t\tComputing Gram matrix: ", end="")
    gm += gram_matrix(
        train_set_kmer, validation_set_kmer, test_set_kmer, sparse_gaussian
    )
    print("\r\t\tGram matrix computed.       ")
...
final_predicitons.to_csv(title, sep=",", encoding="utf-8", index=True)
print(style.italic + style.blue + "\tPrediction saved under " + title + style.normal)
